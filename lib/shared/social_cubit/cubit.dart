  ...
  class SocialCubit  extends Cubit<SocialAppStates>
{
  ...
  void updateIiLikedThisPost() {
    FirebaseFirestore.instance.collection('posts').snapshots().listen((event) {
      for (var postDoc in event.docs)
      {
        final postId = postDoc.id;

        // Listen to the likes collection within each post
        postDoc.reference.collection('likes').snapshots().listen((likeEvent) {
          bool userLiked = false;

          // Check if the current user ID exists in the likes collection
          for (var likeDoc in likeEvent.docs)
          {
            if (likeDoc.id == userModel?.uId)
            {
              // If the user has liked the post, get the 'like' field's value
              userLiked = likeDoc.data()['like'] ?? false;
              break;
            }
          }

          // Update the 'iLikedThisPost' field in the post document
          FirebaseFirestore.instance.collection('posts').doc(postId).update({
            'iLikedThisPost': userLiked,
          });
        });
      }
    });
  ...
  void sendMessage({required String? name,required String? image,required String? receiverUid,required String message,required String dateTime,required String date,required String time, required String fullDate })
  {
